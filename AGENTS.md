...
stdout is a single JSON document (`metadata`, `transcript`, `ocrResults`, `timeline`, `slides`, `warnings`, `artifacts`, and `frames` as `{ time, filePath, mimeType }` → JPEG key frames on disk). `slides` pairs each OCR'd slide with the narration spoken while it was on screen (`ocrText` + `narration`, windowed `[seconds, endSeconds)`), which is the fastest way to answer "what did they say about this slide". `artifacts` names everything written into `--out`: `extracted_audio.wav` (16kHz mono 16-bit, the whisper input) and `extracted_audio.mp3`, `transcript.txt` and a timestamped `transcript.json`, `slides/` (the frame JPEGs as `slide-NN.jpg` plus `slides.json`), and the final synchronized timeline as `timeline.json` + a readable `timeline.md`. Progress goes to stderr. Read the frame images for visual questions; answer with timestamps. `analyze --help` lists all flags.
